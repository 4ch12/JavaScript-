// Задание #1
// Создайте класс Circle, который имеет поле radius,
// а также метод getArea(), высчитывающий площадь 
// круга и округляющий её до двух знаков после запятой. 
...
// Также он имеет метод priceWithDiscout(), который возвращает
// обновленную цену с учетом переданной скидки (в процентах). 
// Метод не изменяет само значение price в объекте, 
// а только выводит обновленное.  
...
// addFriend() — принимает имя нового 
// друга и добавляет его в массив friends;
// showFriends() — выводит в консоль 
// строку со всеми друзьями через запятую.
// Сделайте так, чтобы код ниже работал:
...
// 2. Создайте еще 2 класса, которые будут наследоваться от класса
// Animal — Cat, Dog. Переопределите для них метод makeSound(),
...
// foo.bar();
// foo.baz();
// Проанализируйте результаты вызовов и определите для себя, 
...
// }
// Отредактируйте код так, чтобы при его работе
//  выводились корректные номера коробок. Функцию produceBox() править нельзя. 